...
const IndividualForm = ({ onSubmit, onBack, isLoading }: { onSubmit: (data: IndividualFormData) => void; onBack: () => void; isLoading: boolean }) => {
  const { t } = useTranslation();
  const { register, handleSubmit, formState: { errors } } = useForm<IndividualFormData>({ resolver: zodResolver(individualSchema) });

  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
      <button type="button" onClick={onBack} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-4 transition-colors">
        <ArrowLeft className="w-4 h-4" /> {t("partner.hostReg.back")}
      </button>
...
const CompanyForm = ({ onSubmit, onBack, isLoading }: { onSubmit: (data: CompanyFormData) => void; onBack: () => void; isLoading: boolean }) => {
  const { t } = useTranslation();
  const { register, handleSubmit, formState: { errors } } = useForm<CompanyFormData>({ resolver: zodResolver(companySchema) });
