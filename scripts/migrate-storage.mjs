 ...
 * 
 * Supports both legacy JWT service_role keys AND new sb_secret_* keys
 * (including projects using ECC P-256 / ES256 JWT signing).
 * 
 * Usage:
 *   1. Install dependency:  npm install @supabase/supabase-js
 *   2. Set environment variables:
 *      export NEW_SUPABASE_URL="https://your-project.supabase.co"
 *      export NEW_SUPABASE_SERVICE_ROLE_KEY="sb_secret_... or eyJ..."
...
if (!NEW_SUPABASE_URL || !NEW_SUPABASE_KEY) {
  ...
  console.error("   export NEW_SUPABASE_SERVICE_ROLE_KEY='sb_secret_... or eyJ...'");
  process.exit(1);
}
...
if (cleanKey.startsWith("sb_publishable_")) {
  console.error("❌ Publishable keys can't create buckets or upload files.");
  console.error("   Use sb_secret_* or legacy service_role JWT.");
  process.exit(1);
}
...
// ─── Preflight: verify auth works ─────────────────────────────────────
async function preflightAuth() {
  console.log("\n🔑 Testing authentication...");
  const { data, error } = await newSupabase.storage.listBuckets();
  ...
  console.log(`✅ Auth OK — found ${data.length} existing bucket(s)`);
}
...
async function ensureBuckets() {
  ...
  for (const config of BUCKET_CONFIGS) {
    const { error } = await newSupabase.storage.createBucket(config.name, {
...
async function main() {
  ...
  for (const [bucket, files] of Object.entries(PUBLIC_FILES)) {
    ...
    for (const file of files) {
      total++;
      if (await migrateFile(bucket, file)) success++;
    }
  }
